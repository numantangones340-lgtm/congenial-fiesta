...
class GuitarAmpRecorderApp:
    ...
    def refresh_recent_exports(self) -> None:
        ...
        for index, path in enumerate(recent_files):
            label = f"- {path.name}"
            if index == 0:
                label += " (En yeni export; 'Son Dosyayi Finder'da Goster')"
            lines.append(label)
        hidden_count = max(0, len(all_audio_files) - len(recent_files))
        if hidden_count:
            lines.append(
                f"- ... +{hidden_count} ses dosyasi ('Klasoru Ac')"
            )
        if summary_line:
            lines.append(summary_line)
        self.recent_exports_text.set("\n".join(lines))
    ...
    def refresh_recent_exports_from_action(self) -> None:
        ...
        if not audio_files:
            if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
                self.set_status(
                    ...
                    "Ozet hazir; 'Son Oturum Ozetini Ac' ile acabilirsiniz."
                )
            else:
                self.set_status(
                    "Son ciktilar yenilendi. Gorunen: bos. "
        ...
        summary_suffix = ""
        if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
            summary_suffix = " Ozet hazir; 'Son Oturum Ozetini Ac' ile acabilirsiniz."
        self.set_status(
