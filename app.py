...
class GuitarAmpRecorderApp:
    ...
    def refresh_recent_exports(self) -> None:
        ...
        if not recent_files:
            ...
            if summary_line:
                ...
                lines.append("Export yok. Kayittan sonra burada.")
            self.recent_exports_text.set("\n".join(lines))
            self.refresh_recent_output_buttons()
            return
    ...
    def refresh_recent_exports_from_action(self) -> None:
        ...
        if not output_dir.exists():
            self.set_status(
                f"Liste guncel. Cikis klasoru bulunamadi: {self.format_display_path(output_dir)}. "
                "'Klasoru Ac' ile yeniden olusturabilir ve Finder'da acabilirsiniz."
            )
            return
        audio_files = [
        ...
        self.set_status(
            f"Liste guncel. {len(audio_files)} ses dosyasi.{visibility_suffix}{sort_suffix}{summary_suffix}"
        )
