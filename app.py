...
class GuitarAmpRecorderApp:
    ...
    def refresh_recent_exports(self) -> None:
        ...
        if not recent_files:
            ...
            if summary_line:
                lines.append("Henuz ses kaydi yok. Alttaki ozeti simdi acabilirsiniz.")
                lines.append(summary_line)
            else:
                lines.append("Henuz ses kaydi yok. Yeni kayitlar burada listelenecek.")
            self.recent_exports_text.set("\n".join(lines))
            self.refresh_recent_output_buttons()
            return
    ...
    def refresh_recent_exports_from_action(self) -> None:
        ...
        if not audio_files:
            if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
                self.set_status(
                    "Guncel. "
                    "Ozet hazir. Simdi acabilirsiniz."
                )
            else:
                self.set_status(
                    "Guncel. "
                    "Yeni kayitlar burada listelenecek."
                )
            return
        ...
        summary_suffix = ""
        if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
            summary_suffix = " Ozet hazir. Simdi acabilirsiniz."
        self.set_status(
