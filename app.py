...
class GuitarAmpRecorderApp:
    ...
    def refresh_recent_exports_from_action(self) -> None:
        ...
        if not audio_files:
            if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
                self.set_status("Son ciktilar yenilendi. Liste bos, son oturum ozeti hazir ve acilabilir.")
            else:
                self.set_status("Son ciktilar yenilendi. Liste bos, yeni kayitlardan sonra ciktilar burada gorunecek.")
            return
    ...
    def recent_session_summary_line(self, output_dir: Path) -> str:
        ...
        if summary_path is None:
            return ""
        return "- session_summary.json (Son oturum ozeti, acilabilir)"
