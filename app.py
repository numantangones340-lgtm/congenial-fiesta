...
class GuitarAmpRecorderApp:
    ...
    def refresh_recent_exports(self) -> None:
        ...
        if len(all_audio_files) > len(recent_files):
            ...
            count_line += " | Listede: tek ses dosyasi"
        elif recent_files:
            count_line += " | Listede: tum ses dosyalari"
        else:
            count_line += " | Ses dosyasi listesi bos"
            if summary_line:
                count_line += " | Yalnizca son oturum ozeti dosyasi hazir"
        if len(recent_files) > 1:
            count_line += " | Siralama: en yeni ustte"
        if not recent_files:
            self.last_export_path = None
        ...
        for index, path in enumerate(recent_files):
            label = f"- {path.name}"
            if index == 0:
                label += " (Son export dosyasi; 'Son Dosyayi Finder'da Goster')"
            lines.append(label)
    ...
    def refresh_recent_exports_from_action(self) -> None:
        ...
        if not audio_files:
            if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
                self.set_status(
                    "Son ciktilar yenilendi. Ses dosyasi listesi bos. "
                    "Yalnizca son oturum ozeti dosyasi hazir; 'Son Oturum Ozetini Ac' ile acabilirsiniz."
                )
            else:
                self.set_status(
                    "Son ciktilar yenilendi. Ses dosyasi listesi bos. "
        ...
        if len(audio_files) > shown_count:
            ...
            visibility_suffix = " Listede: tek ses dosyasi."
        else:
            visibility_suffix = " Listede: tum ses dosyalari."
        sort_suffix = " Siralama: en yeni ustte." if shown_count > 1 else ""
        summary_suffix = ""
        if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
            summary_suffix = " Son oturum ozeti dosyasi da hazir; 'Son Oturum Ozetini Ac' ile acabilirsiniz."
        self.set_status(
    ...
    def recent_session_summary_line(self, output_dir: Path) -> str:
        ...
        if summary_path is None:
            return ""
        return "- session_summary.json (Son oturum ozeti dosyasi; 'Son Oturum Ozetini Ac')"
