...
class GuitarAmpRecorderApp:
    ...
    def refresh_recent_exports(self) -> None:
        ...
        if len(all_audio_files) > len(recent_files):
            ...
            if summary_line:
                count_line += " | Yalnizca en yeni oturum ozeti dosyasi hazir"
        ...
        if hidden_count:
            lines.append(
                f"- ... Listede yok: {hidden_count} ses dosyasi (Tum ses dosyalari icin: 'Klasoru Ac')"
            )
        if summary_line:
            lines.append(summary_line)
        self.recent_exports_text.set("\n".join(lines))
        ...
        if not audio_files:
            if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
                self.set_status(
                    ...
                    "Yalnizca en yeni oturum ozeti dosyasi hazir; 'Son Oturum Ozetini Ac' ile acabilirsiniz."
                )
            else:
                self.set_status(
                    "Son ciktilar yenilendi. Listede yok: 0 ses dosyasi. "
