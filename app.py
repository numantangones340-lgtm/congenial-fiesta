...
class GuitarAmpRecorderApp:
    ...
    def refresh_recent_exports(self) -> None:
        ...
        count_line = f"Toplam ses: {len(all_audio_files)}"
        if len(all_audio_files) > len(recent_files):
            ...
            if summary_line:
                count_line += " | En yeni ozet dosyasi hazir"
        ...
        if not recent_files:
            ...
            if summary_line:
                lines.append("Ses dosyasi yok. Asagidaki ozet dosyasini acabilirsiniz.")
        ...
        for index, path in enumerate(recent_files):
            label = f"- {path.name}"
            if index == 0:
                label += " (En yeni export dosyasi; 'Son Dosyayi Finder'da Goster')"
            lines.append(label)
        hidden_count = max(0, len(all_audio_files) - len(recent_files))
        if hidden_count:
            lines.append(
                f"- ... Ek dosyalar: {hidden_count} ses dosyasi (Tum ses dosyalari: 'Klasoru Ac')"
            )
        if summary_line:
            lines.append(summary_line)
        self.recent_exports_text.set("\n".join(lines))
    ...
    def refresh_recent_exports_from_action(self) -> None:
        ...
        if not audio_files:
            if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
                self.set_status(
                    ...
                    "En yeni ozet dosyasi hazir; 'Son Oturum Ozetini Ac' ile acabilirsiniz."
                )
            else:
                self.set_status(
                    "Son ciktilar yenilendi. Gorunen: yok. "
        ...
        summary_suffix = ""
        if self.last_session_summary_path is not None and self.last_session_summary_path.exists():
            summary_suffix = " En yeni ozet dosyasi hazir; 'Son Oturum Ozetini Ac' ile acabilirsiniz."
        self.set_status(
    ...
    def recent_session_summary_line(self, output_dir: Path) -> str:
        ...
        if summary_path is None:
            return ""
        return "- session_summary.json (En yeni ozet dosyasi; 'Son Oturum Ozetini Ac')"
