...
class ExportAndDeviceViewTests(unittest.TestCase):
    ...
    def test_recent_output_open_status_status_text_formats_filename(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)
        path = Path("/tmp/take.wav")

        self.assertEqual(
            recorder.recent_output_open_status_status_text("Son export Finder'da gosteriliyor", path),
            "Son export Finder'da gosteriliyor: take.wav",
        )

    def test_recent_output_open_status_status_prefix_returns_copy(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_status_status_prefix("Son export Finder'da gosteriliyor"),
            "Son export Finder'da gosteriliyor",
        )

    def test_recent_output_open_status_status_target_text_returns_filename(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_status_status_target_text(Path("/tmp/take.wav")),
            "take.wav",
        )
    ...
    def test_recent_output_open_status_detail_text_returns_filename(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_status_detail_text(Path("/tmp/take.wav")),
            "take.wav",
        )
    ...
    def test_output_dir_open_status_detail_text_returns_dir_text(self) -> None:
        recorder = self.make_app()
        output_dir = Path("/tmp/new-session-folder")
        recorder.recent_output_dir_text = mock.Mock(return_value="~/new-session-folder")

        self.assertEqual(
            recorder.output_dir_open_status_detail_text(output_dir),
            "~/new-session-folder",
        )
        recorder.recent_output_dir_text.assert_called_once_with(output_dir)
    ...
    def test_output_dir_open_status_status_target_text_returns_dir_text(self) -> None:
        recorder = self.make_app()
        output_dir = Path("/tmp/new-session-folder")
        recorder.recent_output_dir_text = mock.Mock(return_value="~/new-session-folder")

        self.assertEqual(
            recorder.output_dir_open_status_status_target_text(output_dir),
            "~/new-session-folder",
        )
        recorder.recent_output_dir_text.assert_called_once_with(output_dir)

    def test_output_dir_open_status_status_detail_text_returns_dir_text(self) -> None:
        recorder = self.make_app()
        output_dir = Path("/tmp/new-session-folder")
        recorder.recent_output_dir_text = mock.Mock(return_value="~/new-session-folder")

        self.assertEqual(
            recorder.output_dir_open_status_status_detail_text(output_dir),
            "~/new-session-folder",
        )
        recorder.recent_output_dir_text.assert_called_once_with(output_dir)
    ...
    def test_ensure_output_dir_exists_creates_directory(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)
        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "new-session-folder"

            recorder.ensure_output_dir_exists(output_dir)
    ...
    def test_output_dir_open_error_status_detail_text_returns_exception_text(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.output_dir_open_error_status_detail_text(RuntimeError("boom")),
            "boom",
        )

    def test_output_dir_open_error_status_prefix_returns_copy(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(recorder.output_dir_open_error_status_prefix(), "Klasor acilamadi")
    ...
    def test_open_command_text_returns_path_string(self) -> None:
        ...
        self.assertEqual(recorder.open_command_text(path), "/tmp/demo-output")

    def test_open_command_parts_returns_prefix_and_text(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)
        path = Path("/tmp/demo-output")

        self.assertEqual(
            recorder.open_command_parts(path),
            (["open"], "/tmp/demo-output"),
        )

    def test_open_command_parts_with_prefix_returns_given_prefix_and_text(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)
        path = Path("/tmp/demo-output")

        self.assertEqual(
            recorder.open_command_parts_with_prefix(["open", "-R"], path),
            (["open", "-R"], "/tmp/demo-output"),
        )
    ...
    def test_output_dir_open_command_parts_returns_prefix_and_text(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)
        output_dir = Path("/tmp/demo-output")

        self.assertEqual(
            recorder.output_dir_open_command_parts(output_dir),
            (["open"], "/tmp/demo-output"),
        )

    def test_recent_output_open_command_reveal_suffix_matches_mode(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(recorder.recent_output_open_command_reveal_suffix(True), ["-R"])
        self.assertEqual(recorder.recent_output_open_command_reveal_suffix(False), [])
    ...
    def test_perform_recent_output_open_runs_command_and_reports_success(self) -> None:
        recorder = self.make_app()
        export_path = Path("/tmp/take.wav")
    ...
    def test_perform_recent_output_target_open_dispatches_resolved_path(self) -> None:
        ...
        recorder.dispatch_recent_output_target_open = mock.Mock()
        ...
        recorder.dispatch_recent_output_target_open.assert_called_once_with(
            export_path,
            "last_export_path",
            "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            "Son export Finder'da gosteriliyor",
    ...
    def test_dispatch_recent_output_target_open_delegates_to_resolved_handler(self) -> None:
        recorder = self.make_app()
        export_path = Path("/tmp/take.wav")
        recorder.handle_resolved_recent_output_target_for_open = mock.Mock()

        recorder.dispatch_recent_output_target_open(
            export_path,
            "last_export_path",
            "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            "Son export Finder'da gosteriliyor",
    ...
    def test_recent_output_open_error_status_prefix_returns_copy(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_error_status_prefix("Finder acilamadi"),
            "Finder acilamadi",
        )
    ...
    def test_open_recent_output_from_args_factory_delegates_to_args_opener(self) -> None:
        recorder = self.make_app()
        recorder.open_recent_output_from_args = mock.Mock()
        open_args = {
            "attribute_name": "last_export_path",
            "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            "success_prefix": "Son export Finder'da gosteriliyor",
        ...
        recorder.open_recent_output_from_args_factory(open_args_factory)
    ...
    def test_recent_output_open_args_returns_payload(self) -> None:
        ...
        self.assertEqual(
            recorder.recent_output_open_args(
                attribute_name="last_export_path",
                missing_message="Son export dosyasi bulunamadi; son ciktilar yenilendi.",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
    ...
    def test_recent_output_missing_message_returns_shared_copy(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_missing_message("Son export dosyasi"),
            "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
        )

    def test_recent_output_open_message_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_message_args(
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
            {
    ...
    def test_recent_output_open_target_args_returns_target_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_args(
                attribute_name="last_export_path",
    ...
    def test_recent_output_payload_args_returns_payload_copy(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)
        payload = {
            "attribute_name": "last_export_path",
            "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
        ...
        resolved = recorder.recent_output_payload_args(payload)
    ...
    def test_recent_output_open_target_args_payload_returns_target_payload(self) -> None:
        ...
        self.assertEqual(
            recorder.recent_output_open_target_args_payload(
                attribute_name="last_export_path",
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
    ...
    def test_recent_output_open_target_args_parts_returns_target_parts(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_args_parts(
                attribute_name="last_export_path",
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
    ...
    def test_recent_output_attribute_args_returns_attribute_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_attribute_args("last_export_path"),
            {
                "attribute_name": "last_export_path",
            },
        )

    def test_recent_output_open_target_base_args_returns_attribute_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_base_args("last_export_path"),
            {
                "attribute_name": "last_export_path",
            },
        )
    ...
    def test_recent_output_open_target_args_extra_args_returns_message_payload(self) -> None:
        ...
        self.assertEqual(
            recorder.recent_output_open_target_args_extra_args(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_attribute_args_returns_attribute_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_attribute_args("last_export_path"),
            {
                "attribute_name": "last_export_path",
            },
        )

    def test_recent_output_open_target_extra_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_returns_message_parts(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_returns_message_parts(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_message_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_message_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_message_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_result_message_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts_result_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_message_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_result_message_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_parts_message_parts_result_parts_result_result_result_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_terminal_message_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_terminal_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_terminal_parts_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_terminal_parts_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_terminal_result_message_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_terminal_result_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_terminal_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_terminal_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_terminal_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_terminal_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_returns_message_parts(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_result_parts_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_result(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_parts_returns_message_parts(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_parts_result_result_result_result_result_result_result_result_result_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_base_args_returns_message_payload(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_base_args(
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
            {
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_result_extra_args_returns_message_payload(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_result_extra_args(
                "Son export dosyasi"
            ),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_base_args_returns_message_payload(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_base_args(
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
            {
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_parts_extra_args_returns_message_payload(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_parts_extra_args(
                "Son export dosyasi"
            ),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_base_args_returns_message_payload(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_base_args(
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
            {
    ...
    def test_recent_output_open_target_extra_args_message_parts_result_extra_args_returns_message_payload(
        self,
    ) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_result_extra_args(
                "Son export dosyasi"
            ),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
    ...
    def test_recent_output_open_target_extra_args_message_parts_base_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_base_args(
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
            {
    ...
    def test_recent_output_open_target_extra_args_message_parts_extra_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_parts_extra_args(
                "Son export dosyasi"
            ),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
    ...
    def test_recent_output_open_target_extra_args_base_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_base_args(
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
            {
    ...
    def test_recent_output_open_target_extra_args_message_base_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_base_args(
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
            {
    ...
    def test_recent_output_open_target_extra_args_message_extra_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_message_extra_args("Son export dosyasi"),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            },
        )

    def test_recent_output_open_target_extra_args_extra_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_extra_args("Son export dosyasi"),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            },
        )

    def test_recent_output_open_target_extra_args_missing_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_extra_args_missing_args("Son export dosyasi"),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            },
        )

    def test_recent_output_open_target_message_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_message_args(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_message_parts_returns_message_parts(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_message_parts(
                target_name="Son export dosyasi",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
            ),
    ...
    def test_recent_output_open_target_message_missing_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_target_message_missing_args("Son export dosyasi"),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            },
        )
    ...
    def test_recent_output_message_args_with_missing_returns_message_payload(self) -> None:
        ...
        self.assertEqual(
            recorder.recent_output_message_args_with_missing(
                {
                    "success_prefix": "Son export Finder'da gosteriliyor",
                    "error_prefix": "Finder acilamadi",
                },
    ...
    def test_recent_output_message_args_parts_returns_message_parts(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_message_args_parts(
                {
                    "success_prefix": "Son export Finder'da gosteriliyor",
                    "error_prefix": "Finder acilamadi",
                },
    ...
    def test_recent_output_message_base_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_message_base_args(
                {
                    "success_prefix": "Son export Finder'da gosteriliyor",
                    "error_prefix": "Finder acilamadi",
                }
    ...
    def test_recent_output_message_extra_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_message_extra_args("Son export dosyasi"),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            },
        )

    def test_recent_output_message_missing_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_message_missing_args("Son export dosyasi"),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            },
        )

    def test_recent_output_message_args_returns_message_copy(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)
        message_args = {
            "success_prefix": "Son export Finder'da gosteriliyor",
            "error_prefix": "Finder acilamadi",
        ...
        resolved = recorder.recent_output_message_args(message_args)

        self.assertEqual(resolved, message_args)
        self.assertIsNot(resolved, message_args)

    def test_recent_output_missing_message_args_returns_message_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_missing_message_args("Son export dosyasi"),
            {
                "missing_message": "Son export dosyasi bulunamadi; son ciktilar yenilendi.",
            },
        )

    def test_recent_output_open_args_payload_returns_base_payload(self) -> None:
        recorder = app.GuitarAmpRecorderApp.__new__(app.GuitarAmpRecorderApp)

        self.assertEqual(
            recorder.recent_output_open_args_payload(
                attribute_name="last_export_path",
                missing_message="Son export dosyasi bulunamadi; son ciktilar yenilendi.",
                success_prefix="Son export Finder'da gosteriliyor",
                error_prefix="Finder acilamadi",
